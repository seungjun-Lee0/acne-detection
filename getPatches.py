...
PREDICTOR_PATH = "C:/Users/sjun0/OneDrive/Desktop/acne_detect/models/shape_predictor_68_face_landmarks.dat"
eye_cascade = cv2.CascadeClassifier('C:/Users/sjun0/OneDrive/Desktop/acne_detect/models/haarcascade_eye.xml')
...
def read_imgURL(URL):
    with Image.open(URL) as file:
        with open('temp.jpg', 'wb') as f:
            f.write(image_to_byte_array(file))
...
def extract_patches(imagefile, dimension_dict, face_loc_dict, image_dim, croppedFaces_Dir, i, image_name):
    ...
    if face_detected:
        ...
        forehead_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + key_name +".png")
        ...
        chin_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + key_name +".png")
        ...
        key_name = 'landmark_nose'
        if(nose_x_min >= 0 and nose_x_max >= 0 and nose_y_min >= 0 and nose_y_max >= 0):
            dimension_dict[key_name] = [chin_x_min, chin_x_max, chin_y_min, chin_y_max]
            nose_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + key_name +".png")
        ...
        if right_face:
            ...
            cheek_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + key_name +".png")
    ...
    if not face_detected:
        ...
        if max_area >= min_eye_area:
            ...
            forehead_min_y = int(forehead_min_y)
            forehead_max_y = int(forehead_max_y)
            forehead_ok = False
            # Get the forehead region
            if forehead_max_y - forehead_min_y >= 0.7 * eyes1[0, 3]:
                forehead_ok = True
                forehead_min_x = int(forehead_min_x)
                forehead_max_x = int(forehead_max_x)
            ...
            if face_direction == "left":
                cheek_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + "oneeye_lc" +".png")
            elif face_direction == "right":
                cheek_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + "oneeye_rc" +".png")
            else:
                cheek_file_name = join(croppedFaces_Dir, i + "_" + image_name + "_" + "oneeye_cheek" +".png")
            imageio.imwrite(cheek_file_name, cheek_region)
    
    
    if (not face_detected) and (not eye_detected):
        print("No chin or forehead detected, output nothing")
        #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        #outfile = join(croppedFaces_Dir, i + "_" + image_name + "_"+".png")
